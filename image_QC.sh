...
codedir=${HOME}/Dropbox/Tithub/tractography
basedir="$(pwd -P)"
...
if [ ! -f ${basedir}/diffusion/image_QC.txt ] ;
then
    echo "making output files"
    touch image_QC.txt
    touch image_QC_vis.txt
else
    echo "output files already exists - image_QC.sh has probably been run already"
    if [ "$overwrite" == 1 ] ;
    then
...
log=${basedir}/diffusion/image_QC.txt
log_vis=${basedir}/diffusion/image_QC_vis.txt
echo $(date) >> ${log}
echo $(date) >> ${log_vis}
echo "${0}" >> ${log}
echo "${0}" >> ${log_vis}
echo "Starting image_QC.sh"
echo "" >> ${log}
echo "" >> ${log_vis}
...
echo "" >> ${log_vis}
echo "fsleyes ${basedir}/bet/T1_brain.nii.gz ${basedir}/diffusion/structural.anat/T1_biascorr_brain.nii.gz ${basedir}/diffusion/ants_brains/BrainExtractionBrain.nii.gz " >> ${log_vis}
echo "" >> ${log_vis}
...
fslmaths bet/T1_brain_outskin_mask.nii.gz -mul -1 -add 1 bet/invmask
...
echo "" >> ${log_vis}
echo "fsleyes FDT/dti_FA dti_tensor.nii.gz -ot tensor" >> ${log_vis}
echo "" >> ${log_vis}
...
echo "" >> ${log_vis}
echo "fsleyes bpx.bedpostX/mean_f1samples bpx.bedpostX/dyads1 -ot linevector bpx.bedpostX/dyads2_thr0.05 -ot linevector" >> ${log_vis}
echo "" >> ${log_vis}
...
echo "dyads" >> ${log_vis}
echo "" >> ${log_vis}
echo "fsleyes bpx.bedpostX/mean_fsumsamples.nii.gz bpx.bedpostX/dyads1.nii.gz -ot linevector -xc 1 0 0 -yc 1 0 0 -zc 1 0 0 -lw 2 bpx.bedpostX/dyads2_thr0.05.nii.gz -ot linevector -xc 0 1 0 -yc 0 1 0 -zc 0 1 0 -lw 2 bpx.bedpostX/dyads3_thr0.05.nii.gz -ot linevector -xc 0 0 1 -yc 0 0 1 -zc 0 0 1 -lw 2" >> ${log_vis}
echo "" >> ${log_vis}
...
echo "" >> ${log}
echo "" >> ${log_vis}
echo "all done with image_QC.sh"
echo "all done with image_QC.sh" >> ${log}
echo "all done with image_QC.sh" >> ${log_vis}
echo $(date) >> ${log}
echo $(date) >> ${log_vis}
echo "" >> ${log}
echo "" >> ${log_vis}
